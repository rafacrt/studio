...
const formatMinutes = (totalMinutes: number | undefined): string => {
    if (totalMinutes === undefined || totalMinutes === null || totalMinutes < 0) {
        return <span className="text-muted">N/D</span>; // Not Available or Error
    }
  ...
  const completedOS = useMemo(() => {
    ...
    filtered.sort((a, b) => {
        ...
        if (sortBy === 'dataFinalizacao') {
            compareResult = parseISO(valA!).getTime() - parseISO(valB!).getTime();
            ...
            compareResult = parseInt(valA as string, 10) - parseInt(valB as string, 10);
      ...
      <div className="card shadow-sm">
        ...
        <div className="card-body p-0"> {/* Remove padding for full-width table */}
            ...
            <div className="table-responsive">
              <table className="table table-striped table-hover mb-0"> {/* Added table-hover, removed bottom margin */}
                <thead className="table-light">
                  <tr>
                    <th scope="col" onClick={() => handleSort('numero')} style={{ cursor: 'pointer' }}>
                    ...
                    <th scope="col" onClick={() => handleSort('dataFinalizacao')} style={{ cursor: 'pointer' }}>
                         <CheckCircle2 size={14} className="me-1" /> Data Finalização {renderSortIcon('dataFinalizacao')}
                    </th>
                    <th scope="col" onClick={() => handleSort('tempoProducaoMinutos')} style={{ cursor: 'pointer' }}>
         ...
         <div className="card-footer text-muted small">
            Tempo de produção é calculado do momento que a OS entra em "Em Produção" até ser "Finalizada".
        </div>
      </div>
    </AuthenticatedLayout>
  );
...
// Helper to safely return a value for sorting
function getSortValue<T>(obj: T, key: keyof T): any {
